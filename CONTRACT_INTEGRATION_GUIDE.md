...
1. **获取某个用户的挂售和挂租的 NFT 列表**
2. **所有用户的挂售和挂租的 NFT 列表**
3. **用户之间购买、租赁 NFT**
4. **用户所持有的 NFT（已注册+已租赁）**
5. **当前用户的所有交易记录**
    ...
    <div>
      ...
      {nfts.map((nft) => (
        <div key={nft.tokenId}>
          ...
          <p>名称: {nft.name}</p>
          <p>拥有者: {nft.owner}</p>
          {nft.saleInfo?.isForSale && <p>挂售价格: {nft.saleInfo.price} ETH</p>}
        </div>
      ))}
    </div>
  );
...
4. **权限检查**：某些操作需要用户授权或拥有相应的 NFT
5. **Gas 费用**：所有写入操作都需要支付 Gas 费用
